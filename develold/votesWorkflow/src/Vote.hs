...
projectRegister p= do
   print p
   handleJust errorCalls handle $ do
    withVResources  [Rp uProject{pname=pname p},npubs,nvis] process
    return True
  where
    npubs= uRl {lname=pubProjects}
    nvis = uRl {lname=visProjects}
    handle e=do
    	logEntry $ "Warning: " ++e
    	return False
    ...
    process[Nothing,pubs,vis]= [Rp p, if public p  then lpubs pubs else npubs, if visible p then lvis vis else nvis]

    process r =  error $ "projectRegister: "++name++" withVResources returned: "++show r 

    lpubs Nothing    = Rl{lname=pubProjects,lcontent=[name]}
    lpubs (Just pubs)= pubs{lcontent=name:lcontent pubs}

    
    lvis Nothing   = Rl{lname=visProjects,lcontent=[name]}
    lvis (Just vis)= vis{lcontent=name:lcontent vis}
...
voteSlots pr s= 
        ...
        in  if delta<=1 then v else array(0, lusers-1+buffer) $ assocs v ++ spare
        

voteSlotsSum  s=
      let
          delta= case options s of
                     Approbal -> 4
                     ChooseOptions _ n _ ->   n+2 -- 2: not voted + voted unconstitutional

          spare= (0,1){-the author has not voted-}:(take (delta-1) $ zip [1,2..] (repeat  0))
      in  array(0, delta-1)  spare
...
hasSubjectUser subject us = find (\s -> (uObject us)== subject)(usubjects us)
      ...
      lsumVotes= accum (+) (sumVotes  sub)  $ res `debug` show res  
                    ...
                    where
                       ...
                       swap (Priority p, IndexUser i)
                           -- sum a new vote and subtrac from the previous option voted
...
blankUser    = User  "" "" 0 [] [] [] 
blankSubject = Subject 0  Propose  "" "" "" "Ordinary" [] [] Draft (Str "")  Approbal
...
logHandle= unsafePerformIO $ do
         h<- openFile ( dataPath++ "log.log") AppendMode
...
logEntry str= do
                ...
                let cal= calendarTimeToString ct 
                putStr cal
                hPutStr logHandle cal
                putStr  ":"
                hPutStr logHandle ":"
                putStrLn $ str
