...
module Aprobal(reorderProposalAmends, sumVotes,checkApprobal1,getMajorities,getMajorities1) where
...
getMajorities1 sub=
  case actionParams sub of
    VoteDefault maj _-> maj
    _    -> error "no majorities"


checkApprobal1::  Subject  -> Subject
checkApprobal1  sub  
...
reorderProposalAmends project subject= do
        ...
        maj<- getMajorities (sname s)  project
...
closeProposal args@[project,subject,category]= do
    maj<- getMajorities  subject project 
    ...
    doIt _ _  [Nothing]= error ("Proposal not found in database: "++subject)
    doIt vtime span  [Just (Rs s)]= [Rs s{sstatus= Closed $ sstatus s}] 
...
openPropForVote args@[project,subject,category] = do
  ...
  where
  time= fromIntegral.votationTime.unsafePerformIO $ getMajorities subject  project
  openIt [Just (Rs sub)]= 
