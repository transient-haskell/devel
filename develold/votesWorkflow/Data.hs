...
data Subject= Subject
                ...
                ,daysBefore      :: Int                 -- days before for the current action
...
instance (Typeable   a) =>  Read (Lambda a) where
   readsPrec _ st=  [(Lambda str2 $! lambda , str3)] where
     lambda=  unsafePerformIO $ do
                res<- eval_ str2 ["Data.TCache.Dynamic","Data"] [] []["."] 
                case res of
                 Right mv -> case mv of
                                    Just v -> return v  `debug1` "XXXXXXXX evaluated XXXXXXX"
...
type Context=  Action -> ResourceVote -> Workflow IO ResourceVote

newtype Workf= Workf ( Context ->ResourceVote -> Workflow IO ResourceVote) deriving Typeable
newtype Filter = Filter(Context ->ResourceVote -> Bool)  deriving Typeable
newtype CheckApprobal= CheckApprobal (Context -> Subject  -> Subject) deriving Typeable
newtype CheckAmends = CheckAmends (Context -> [ResourceVote] -> [ResourceVote])  deriving Typeable
newtype ExecFunc= ExecFunc (Context -> ResourceVote -> IO  ResourceVote)  deriving Typeable
...
data Action = Remite Group  String --Workflowname   -- remite a un grupo una propuesta con una categoría
               --para Goto
            | Create ResourceVote
--quizá seria  bueno añadir llamadas libres que generen un elemento de Data.hs: Subjects, Users o Groups: Exec (ResourceVote -> IO ResourceVote)
            ...
            | WaitFor (Lambda Filter)
            | StartWorkflow (Lambda Workf)
            deriving (Read, Show)
