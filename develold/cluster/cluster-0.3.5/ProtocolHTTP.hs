...
import Control.Exception (catch, finally,handle, SomeException)
...
instance (Read t, Show t) => Protocol  t URIHold where
   send x (URIHold surl url)= handle (\ (e :: SomeException) -> return Nothing) $ do
          ...
          case M.lookup surl msockets of
           ...
           Just (Sock sock) -> do
                ...
                if open then send1 sock x url
                                 ...
                                 connection <- newConn
                                 putStrLn " SEND: connection was closed"
                                 send1 connection x url
         ...
         if not open then error  "connection was closed"
            ...
            case rrequest of
             ...
             Right req  -> do
                ...
                where
                errProcess (e  :: SomeException) = return $ show e
                respond str=do
                  open <- sIsConnected sock'
