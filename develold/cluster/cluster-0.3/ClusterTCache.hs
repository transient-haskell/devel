...
data Data1 op a = Cluster URIHold (M.Map String Node) NNodes 
...
instance TransMap op a => TransMap  (Op op a) (Data1 op a)  where
  ...
  mapping (ReqAddNode uri id) = reqAddNode uri id where
              ...
              ugenIdNode = unsafePerformIO genIdNode  ::  Id


  mapping (AddNode uri id) = addNode uri id where
          addNode :: URIHold -> Id -> [Maybe (Data1 op a)] -> [Data1 op a]
          addNode uri@(URIHold surl url) id  [Just (Cluster muri map nNodes)]                                   
                 =   [Cluster muri (M.insert surl (Node (NodeNumber nNodes) uri id0 id True) map) (nNodes+1)]  `debug` ("*****addNode*****")
...
instance (Show op, Show a, Read op, Read a,T.IResource a) => T.IResource (Data1 op a )  where 
   keyResource (Cluster _ _ _)    = "Cluster"
...
instance (GetProto  a) => GetProto  (Data1 op a) where
        getProto  "Cluster"        = Cluster u u u where u=undefined 
...
tvio=  reference $ (Cluster (makeURI "http://localhost" )  M.empty 0    ::Data1 op a)                    
...
instance  (Show a, Read a, Show op, Read op
          ,Protocol (URIHold,Id,[Tr (Op op a)]) URIHold 
          ,SyncRepository (Tr (Op op a)) (Data1 op a) Id 
          ,T.IResource (Data1 op a)
          ,TransMap op a
  ...
  setMyNode myuri@(URIHold surl _)= atomically $ do             --mark my node as inactive (don't send messages to myself)
    Cluster _ map m <-  readTVar tv :: STM (Data1 op a)
    let node = case M.lookup surl map of
            Nothing -> errors surl
            Just node -> node 
    let Node{number=NodeNumber x} = node
    let map' = M.insert surl node{active=False} map
    writeTVar tv $ (Cluster myuri map' m :: Data1 op a)
    

    return x
       
  getNodeIds = do
       Cluster _ map _ <- atomically $ readTVar tv :: IO (Data1 op a)
  ...
  setActive (URIHold surl url) bool= atomically $ do
    Cluster mu map m <- (readTVar tv ::  STM (Data1 op a))                            
    case M.lookup surl map of                                        
        Nothing -> return  ()                                                      `debug` ("not found in setActive in map:"++show map)
        Just (Node n uri ls lr active)-> do
            let map' = M.insert surl (Node n uri ls lr bool) map
            writeTVar tv $ (Cluster mu map' m ::Data1 op a)                         `debug` (show surl++" active setted "++show bool)
  ...
  putLastIdSent (URIHold surl url) id= atomically $ do  -- :: nodeId -> Id -> IO()
    ...
    case M.lookup surl map of
        ...
        Just (Node n uri ls lr st)-> do
           ...
           writeTVar tv $ (Cluster mu map' m :: Data1 op a)
...
--addNodeAction ::[t]-> IO()    -- to detect and process transactions that add nodes to the cluster
    ...
    proc= do     
     ...
     case filter  isAddNode  ts of
        ...
        allObjects=do
           ...
           where 
           f mynode(Just (Cluster mu m n))=  Cluster  mu (M.map activate m) n 
                       where 
        ...
        sendAllObjects all uri= do
            ...
            if not active then return () else do
                ...
                send (myuri,id0, [Tr id [] $ AddObjects all ::  Tr (Op op a)]) uri
                return()
            
        isoldactive=do
              Cluster _ map _ <- atomically $ readTVar tv :: IO (Data1 op a)
              let clusternodes=  M.elems map
              NodeNumber mynode <- readMVar myNodeNumber
  ...
  isAlone _= do
        initialized <- clusterInitialized undefined
        if not initialized then return True else do
          Cluster mu m _ <- atomically $ readTVar tv   :: IO (Data1 op a) 
          return $ not $ M.fold (||) False $ M.map active m   

  myNodeId = unsafePerformIO $ newMVar (makeURI "http://localhost")
  ...
  initCluster myurl clusterurl= do 
        ...
        forkIO $  setCallback port (receiveTrans ::(URIHold,Id,[Tr (Op op a)])  -> IO (URIHold,Id,[Tr (Op op a)])) 
        threadDelay 1000000
        Cluster _ _ n' <- atomically $ readTVar tv :: IO (Data1 op a)
        runCluster  10 port  --seconds

        n <- if n' ==0 then createCluster myuri
             else return n'
         
        modifyMVar_ myNodeNumber  (\_->return $  NodeNumber n )                 `debug` "setting myNodeNumber"
        modifyMVar_ myNodeId (\_->return myuri )                                `debug` "setting myNodeId"
          

        return myuri                                                            `debug` "return"

        where
        createCluster myuri=do
          let clusteruri= makeURI clusterurl                                    `debug` ("clusterurl="++ show clusterurl)
          send (myuri, id0, [Tr id0 ["MyNewTrans"] (ReqAddNode  myuri id0):: Tr (Op op a)])  clusteruri >> return () --send a request to a node of the cluster
          threadDelay 1000000
  
          atomically $ do   -- wait until the node has been created
              Cluster _ _ nnodes <- readTVar tv :: STM (Data1 op a)
              if nnodes==0 then retry                                             `debug` "no node" 
                           else return ()                                         `debug` "NODE CREATED"
  
  
          setMyNode myuri                                                         `debug` "setMyNode"
