...
instance (Read t,Show t) => Protocol  t URIHold where
   send ts (URIHold surl url)= do 
         ...
         Just auth -> 
          case M.lookup surl msockets of
           ...
           Just (Sock sock) -> do --open <- sIsConnected sock
                                  {-if open then-} send1 sock ts url -- else  newConn      `debug` "connection was closed"
      ...
      send1 sock ts url=   do
           rresp <- sendHTTP  sock $! genReq ts url                              `debug` (show $genReq ts url)
           case rresp of
              Left str     -> do
                               print str                                            `debug` "****response error*****"
                               return Nothing
              Right resp ->                                                         
                   case rspReason resp of
                        "OK " -> do
                                let ts' = read $ rspBody resp                        
                                return $ Just ts'                                      `debug` "****response received***"
                        msg    -> return Nothing                                       `debug` ( "**** "++msg++" received ****")
           where
           genReq os comp= 
             Request { rqURI = comp
                     , rqMethod= POST
   ...
   setCallback  port f= pwrapper port f1 where
     f1 req= do
       let ts= read $ rqBody req                                                     `debug` ("rqbody="++rqBody req)
       let url= rqURI req
...
{-
         ...
         if not open then  acceptConnections fn sock                               `debug` "connection was closed"
           else do
-}
            ...
            case rrequest of
             ...
             Right req  -> do 
                --msockets <- takeMVar sockets                                       
                let url= rqURI req                                                  
                --putMVar sockets $ M.insert (toString url) (Sock sock') msockets
                --forkIO $ do
                ...
                where
                errProcess e = return "error"
                respond str=respondHTTP sock' Response{rspReason="OK"
