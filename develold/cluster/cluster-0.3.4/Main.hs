...
main = do
  ...
  case args of
    [] -> do
           ...
           let command0 n= "runghc Main.hs " ++ url n ++" Nothing 2>&1 | tee " ++ datdir ++ "sal"++show n
           let command1 (n, m)= "runghc Main.hs " ++url n ++ " \"Just \\\""++url m++"\\\"\"" ++"2>&1 | tee " ++ datdir ++ "sal"++show n
...
test= do
  ...
  id <- genIdNode
  let t= Tr 0 id False [KeyObj "int"]  (Op ( Add 1) )
