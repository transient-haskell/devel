...
instance  (Show a, Read a, Show op, Read op
  ...
  reconnectAction id = do
           AllObjects n all <- return . fromIDyn =<< readTMVar tvAllObjects         `debug` ( "reconnect id="++ show id)
           let (all1,_)= unzip . filter (\(_,mod)->  mod>t) $ nubBy nubBy1 all

           --let t= unsafeCoerce 1
           withSTMResources (map Key1 ( all1 \\ ["PendingTrans","MyNode"]) ) f      `debug`  ("all="++ show all++" all1="++  show all1)
  ...
  applyCSTrans  ts = do
        ...
        MyNode mynode <-  return . fromIDyn =<<  readTMVar tvMyNode         `debug` ( "applyCSTrans TS="++ show ts)
        ...
        let lastRec= firstValid . reverse $ ts
        ...
        putTMVar tvPendingTrans $ toIDyn $ PendingTrans ts3                          `debug` ("applyCSTrans final PendingTrans: "++ show ts3)
...
-}
  ...
  transToSend src   lastRec  = do
    ...
    -- discard transactions that came from that node, that where already sent and the Noop operations not generated directly by this node.
  ...
  respond (MyNode mynode, Node node, tvnode,lastRecFromNode)=  do
      ...
      tosend <- case active node of
         ...
         Active -> do
               ...
               tsToSend <- transToSend (number node) lastRec
