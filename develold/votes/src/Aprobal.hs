{-# OPTIONSB -fglasgow-exts -XDeriveDataTypeable #-}
module Aprobal(reorderProposalAmends, sumVotes, cronList, addSubjectProc, startCronTab, cronExec) where
...
1       si la propuesta ha sido aprobada, el porcentaje de votos debe ser mayor que el asignado a esa categor�a
...
checkApprobal1 project sub
    ...
    | otherwise=defaultCheck
                ...
                -}
    where
    prConf= unsafePerformIO $ getProjectConf project
    majorities= getMajorities1 (category sub) prConf
...
closeProposal args@[project,subject,category]= do
   ...
   where
    ...
    doIt _ _ _ [Nothing,_]= error ("Proposal not found in database: "++subject)
    doIt vtime span scategory [Just (Rs s),crtab]= 
                     case scategory of
                        "Constitutional" ->
...
openPropForVote args@[project,subject,category] = do
  ...
  openIt [Just (Rs sub),lprocs]=
      [Rs sub{sstatus= Processing}
      ,addSubjectProc time CheckApprobalProp sub lprocs ]

  openit [Nothing,_] = error subject++" not found in openProfForVote"
