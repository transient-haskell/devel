...
pageBodyPr:: HSP XML -> String -> String -> String -> HSP XML -> HSP XML  -> HSP XML
pageBodyPr tabs title subtitle header  rigthHtml body= 
    
     <table border="0" width="100%" cellspacing="9" style="border-collapse: collapse" 
              bordercolor="#111111" cellpadding="7">
        <tr><td><% tabs %></td></tr>
        ...
        htmlIntro  intro= <p  id="intro"> <% intro%>    </p>
...
pageBody tabs lang body= pageBodyPr tabs tfreechooser tyuchus tintro  (rigthSide lang) body
    where (tintro,tfreechooser, tyuchus)= case lang of
                                    "es" -> (LangEs.intro, LangEs.freechooser, LangEs.yuchus)
                                    _    -> (Lang.intro, Lang.freechooser, Lang.yuchus)

philosophyPage = do
         let tabs = createTabs [freeChooserName] "Philosophy" ["Groups","last group","last proposal"]
  ...
  where
        ...
        body pr= <p>
                    ...
                    <input type="hidden" name="type" value=types/>
                        ...
                        <p align="center"> 
                           <textarea name="users" rows="10" cols="65" wrap="True" ><%concatMap ("\n"++) $ users pr %> </textarea>
                        </p>
                        <% vsep %>
        ...
        (tpleasevisible, tpleasepublic, tpleaseemails, tpleasetopics, tpleasedescrip, tpleaseentername, tthispagecollect, texplainpublic, texplainvisible, texplaintopics, texplaindescrip,texplainemails,texplainname)= case env->>acceptLang of
                                ...
                                _    -> (Lang.pleasevisible, Lang.pleasepublic, Lang.pleaseemails, Lang.pleasetopics, Lang.pleasedescrip, Lang.pleaseentername, LangEs.thispagecollect, Lang.explainpublic, Lang.explainvisible, Lang.explaintopics,Lang.explaindescrip, Lang.explainemails, Lang.explainname)
...
modifySubjectPage1 env cookies s  msg =do 
   conf <- getProjectConf $ prosname s
   let lcategories= categories conf
                ...
                <% case types of
                        "create" -> -- edit name, category
                            <p><p align="justify"><b><% tpleasename %></b> <% texplainnamesub %> </p>
                                   ...
                                   <%[<p><input type="radio" checked=(if x == category s then True else False) 
                        ...
                        "modify" -> -- no edit name, edit category
                            <p><input type="hidden" name="name" value=(sname s)/>
                                  <% if category s == "Constitutional" then <input type= "hidden" name= "category" value= "Constitutional" />
                                      else <p><p align="justify"> <b> <%tpleasecategory %> </b><% texplaincategory %></p>
		...
		<% case category s /= "Constitutional" of
		       ...
		       False -> <nop/>
                ...
                <%if category s /= "Constitutional" then 
                                        <input type="hidden" name="typeOption" value="approbal" />       
                   else 
                    <p>
                ...
                where
                        ...
                        topicBoxes      = map (\t -> <span><input type="checkbox" name=("topic"++t) value="OFF" checked=(checked t)/><b><% t %>&nbsp;&nbsp;</b></span>) (ptopics pr)
...
userPage env cookies msg=  do
 ...
 --   return Content{mime=page, cookies= cookies
   (projectCookie, env2)<- prCookie env
...
-- to create a cookie for the project
   where
     email= env->>"email"
     project= env->>"project"
     prCookie env = do
        prs <- if null email then return [] else projectListNames  email
   
        let pr= if length prs==1 && null project then head prs
                        else project
        
        if not $ null pr then return ( [("project", pr, "/", "Wed, 31-Dec-2010 12:00:00 GMT")],env)
         else return ([],env)
...
listProjects env msg= do
   ...
   where
        tabs = createTabs [] freeChooserName []
        lang= env->>acceptLang
        notLogged pubpr = 
           <p>
                --<p align="justify"><b><% texplain %></b></p>
                <%vsep%>
        ...
        (tstrListProjects,tyouarenotgroup, tthissectionshows, tpublicinvitedto, tyoupublic, tnogroup, tnamet, tgroupspage, tgroupsmemberof, texplain,tdelegate,tcreateyourgroup,tcreategroup,tauthort,ttopicst,tactions,taskacreator)= 
            case lang of
                    "es" -> (LangEs.strListProjects,LangEs.youarenotgroup, LangEs.thissectionshows, LangEs.publicinvitedto, LangEs.youpublic, LangEs.nogroup, LangEs.namet, LangEs.groupspage, LangEs.groupsmemberof, LangEs.explain,LangEs.delegate,LangEs.createyourgroup,LangEs.creategroup,LangEs.authort, LangEs.topicst,LangEs.actions,LangEs.askacreator)
                    _    -> (Lang.strListProjects,Lang.youarenotgroup, Lang.thissectionshows, Lang.publicinvitedto, Lang.youpublic, Lang.nogroup, Lang.namet, Lang.groupspage, Lang.groupsmemberof, Lang.explain,Lang.delegate,Lang.createyourgroup,Lang.creategroup,Lang.authort, Lang.topicst,Lang.actions, Lang.askacreator)
...
showProject env=        do
        ...
        let tabs = createTabs [freeChooserName] project morep
        return $ page project  $! pageBodyPr tabs project "a group of www.freechooser.com" (showTextContent $ pdescrip pr) (rigthSide lang) content 
  ...
  where
    ...
    showp us pr tnow=do
          jprs <- mapM (listSubject lang project email tnow ) $ psubjects pr
...
listSubject lang project email tnow subject =do
        ...
        if sstatus s == Draft && authors s /=[email] then return Nothing 
          ...
          return.Just  $
                <tr><td><%sname s %></td><td><p><%[<a href= ("mailto:"++auth)><%auth++"&nbsp;"%></a>| auth<-authors s]%></p></td>
...
listProject lang us pr=
        ...
        where
                ...
                editLink        =       <a href= (cgiURL++"?op=pro&project="++namep++"&type=modify")><% tedit %> </a> 
                delLink         =       <a href= (cgiURL++"?op=del&op2=pr&project="++namep)><% tdelete %></a> 
                viewLink        =       <a href= (cgiURL++"?project="++namep)><% tselect %> </a>
                nameLink        =       <a href= (cgiURL++"?project="++namep)><% namep %></a>
                authorLink      =       <a href= ("mailto:"++authord)><% authord %></a> 
                namep           =       pname pr
                authord         =       pauthor pr
...
showSubjectHeader lang s email =
     <span>
                <p><% tauthort++":" %>&nbsp;</p>
                <% [<b><% auth %>&nbsp;</b> | auth <- authors s] %>

                <br/>
                <% ttopicst++": &nbsp;" %>
                <b><% listByComma (Data.topics s) %></b>
 
                <br/>&nbsp;&nbsp; <% if canEdit email s rangev then <% editLink s %> else <span/> %>
    </span>
...
showSubjectPage env= do
     ...
     let   tabs = createTabs [freeChooserName] subject moresub
     return Content{ 
...
showVoted lang hasVoted options = 
    ...
    where
            ...
            (tsubjectvoted, tyouvoted,tnotvoted,tprojectvoted,ttopicvoted) = case lang of
                    ...
                    _    -> (LangEs.subjectvoted, LangEs.youvoted, Lang.notvoted,Lang.projectvoted, Lang.topicvoted)
...
votation lang options votes sumVotes= 
        ...
        where 
                v= elems $ sumVotes 
                percent1 :: Int -> Float
                percent1 x= if  votes == undefined then 0.0 else (fromIntegral x*100)/fromIntegral (upper+1)  where (0,upper)= bounds votes
...
showVoteSubjectPage env cookies msg= do
     ...
     if null email then  return Content{ mime= page subject  $ pageBody tabs lang $ showSubject lang s  rangev email, cookies=[]}
                ...
                return Content{mime= page subject  $ pageBodyPr tabs project "" (showTextContent $ pdescrip pr)  (rigthSide lang)  
     ...
     where
                    ...
                    then 
                        <% showSubject lang s  rangev email %> 
                           ...
                           <p>
                                ...
                                <% if voteCond s rangev 
                                    then
                                          <a href= (cgiURL++"?op=sub&subject="++subject++"&type=amend")><b><% tsuggestmodif %></b></a> `seq`
                  ...
                  <form action=(cgiURL++"?op=vsub") method="post">
                   <p align="center">
                        ...
                        <table tableStyle> 
                            ...
                            <tr><th><% tdeltype %></th><th><% tdelemailt %></th><th><% tchange %></th></tr> 
        ...
        (tsuggestmodif, tstrsubject, tstrproject, tonlyfor,tnone, tnewdeluser,tnewemail,texplaindel, tdeltype, tdelemailt,tdelegate, tdelegatest,tchangebut,tchange)= case env->>acceptLang of
                "es" -> (LangEs.suggestmodif, LangEs.strsubject, LangEs.strproject, LangEs.onlyfor, LangEs.none, LangEs.newdeluser, LangEs.newemail, LangEs.explaindel, LangEs.delemailt, LangEs.delemailt,LangEs.delegate, LangEs.delegatest,LangEs.changebut, LangEs.change)
                _    -> (Lang.suggestmodif, Lang.strsubject, Lang.strproject, Lang.onlyfor, Lang.none, Lang.newdeluser, Lang.newemail, Lang.explaindel, Lang.delemailt, Lang.delemailt,Lang.delegate,Lang.delegatest,Lang.changebut, Lang.change)
