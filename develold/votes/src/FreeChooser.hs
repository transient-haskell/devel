...
sched env=do
     case env of
	  ...
	  _ ->	
		...
		"chp"	-> userPage env [("project", env->>"project", "/", "Wed, 31-Dec-2006 12:00:00 GMT")] ""
...
createSubjectAction env= do
  {-if length ssname < 10 || types=="amend" && length aname < 10 then modifySubjectPage1 env [] (sub 0) tpropmorethan
        ...
        daysBefore <- if category == "Constitutional" && ssname == majorities && types== "create" then return 0
                       else do

	   		maj<-	 getMajorities  category pname
                                	   		
                        return $ votationTime maj  `debug` printdebugsub


	r <- case types of	
	     "create"  -> createSubject ( sub daysBefore) `debug` ( "createSubject, subject= "++printdebugsub)
	     "modify"  -> modifySubject $! sub daysBefore `debug` "modifySubject"
	...
	oldname		=	env ->> "oldname"
	printdebugsub :: String
	printdebugsub=
                                        	   		         sname debugsub++(Data.aname debugsub)
                                        	   		
                                        	   		
                                        	   		         ++show (prosname (sub 0))
                                        				++show (Data.category debugsub)
                                        				++show (Data.authors debugsub)
                                        				++show (Data.topics (sub 0))
                                        				++show (Data.sstatus (sub 0))
                                        				++show (Data.content (sub 0))
                                        	
                                        				++show (Data.options (sub 0))
                                        			        ++show (Data.votes (sub 0))
                                        			        ++show (Data.sumVotes (sub 0))
                                        				++show (Data.lastVote (sub 0))
                                        				++show (Data.daysBefore (sub 0))
	debugsub :: Subject
        debugsub= sub 0
	sub daysb	= 	uSubject
	                         {sname=ssname
				 ...
				 ,options= options
			         ,votes= emptyVotes
			         ,sumVotes= emptySum
				 ,lastVote= flastVote $ fromIntegral daysb
				 ,daysBefore=  daysb
...
createPrAction env
 ...
 where
	pr = Project  dname pauthor pdescrip ltopics lusers lsubjects isPublic isVisibl
	--maj= Subject majorities "" dname  "Constitutional" [pauthor] [] Processing (ConfText defaultConstitution)
	--					Approbal undefined undefined undefined undefined {-updated after edition-}
	maj=Subject
                {sname           = majorities-- proposal name
                ,aname           = "" -- amend name, when applicable
                ,prosname        = dname -- Project name
                ,category        = "Constitutional" 
                ,authors         = [pauthor] 
                ,topics          = [] 
                ,sstatus         = Processing
                ,content         = ConfText defaultConstitution                 -- content of the proposal (different content types)
                ,options         = Approbal                  -- options to vote
                ,votes           = emptyVotes
                ,sumVotes        = emptySum
                ,lastVote        = 0              -- last time to vote in clockTime units
                ,daysBefore      = 0              -- starting time to vote in days before end.
