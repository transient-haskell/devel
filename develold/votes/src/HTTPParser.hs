...
instance Mime (XMLGenT IO XML) where
   mimeType _ = "text/html"


instance Read (XMLGenT m XML)where
	readsPrec _= error "read XML"


instance Show (XMLGenT IO XML) where
	show = unpack . renderXML . unsafePerformIO . unXMLGenT
	           -- renderXML . unsafePerformIO . evalHSP
...
urlDecode :: String -> [(HTTPParser.Name,Value)]
urlDecode s = case readsPrec 0 s of
                 [] -> []
                 ((URLEncoded e,_):_) -> e

env :: Parser [(HTTPParser.Name,Value)]
env = (do n <- urlEncoded
          string "="
          v <- urlEncoded
