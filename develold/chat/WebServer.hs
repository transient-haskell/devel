...
chunkedWrapper :: Integer -> IO String -> IO()
chunkedWrapper port f= rawPwrapper port fn where
   fn h= do
...
makeEnv h= do
		...
		return $ deblank $ (if (headr ->> "REQUEST_METHOD") =="POST" 
                  					then urlDecode  body 
...
scheduler env= 
    ...
    | extension  `isMenberOf` ["hs","lhs","o"] || isUpper $ head name  =  do
         mv<-getMethod env
         case mv of
