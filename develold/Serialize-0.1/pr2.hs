...
instance RefSerialize a =>  RefSerialize (Stat a) where
    ...
    gshowParser  (Stat wfName state index recover sync resource)= do
       ...
       wfStr  <- gshowParser wfName
       statstr <- gshowParser state
       indexstr <- gshowParser index
       recoverstr <- gshowParser recover
       return $ "Stat "++ wfStr ++" "++ statstr ++" " ++ indexstr++" "++ recoverstr++" "++ show sync ++ parsea  
       
    greadParser = choice [rStat, rData, rWorkflows] where --choice between the three 
        rStat= do
              symbol "Stat" 
              wfName <- greadParser
              state <- greadParser :: ST Int
              index <- greadParser :: ST Int
              recover <- greadParser
              sync <- bool
