{-# LANGUAGE Rank2Types,MultiParamTypeClasses, ExistentialQuantification, ScopedTypeVariables, FlexibleInstances, FlexibleContexts, UndecidableInstances #-}
-- some imports
...
--ety :: a -> b 
ety=  id -- dontWorryEverithingisOk
--tdyn :: a -> Dyn
tdyn=  id-- dontWorryEverithingisOk
--fdyn :: Dyn -> a
fdyn = id -- dontWorryEverithingisOk
...
newtype Transient m a = Transient { runTransT :: forall b.(b -> m a) -> m a }
...
react setCallback iob= callCC $ \ret -> do
            st <- get
            liftIO $ setCallback $  \x ->  do runTransState st (ret x) `catch` exceptBack st
                                              iob
            empty

main= keep $  do
   r <- async (return "hello") <> async (return " world")     
   liftIO $ print r     
