...
(<*>) ::  (Subset req' effs) => TRS effs req (a -> b) -> TRS effs' req' a 
      -> TRS (Union effs  effs' :\ Async) (Union req req') b
TR a <*> TR b=  TR $  a A.<*>  b

mempty :: Monoid a => TRS '[] '[] a
mempty = TR M.mempty

(<>) ::  (Monoid a, Subset req' effs) => TRS effs req a -> TRS effs' req' a 
      -> TRS (Union effs  effs') (Union req req') a
TR a <> TR b=  TR $  a M.<>  b

return :: a -> TRS '[] '[] a
return= TR. P.return

(>>=) :: (Subset req' effs) => TRS effs req a 
      -> (a -> TRS effs' req' b) 
